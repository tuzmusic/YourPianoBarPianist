...
//import RealmSwift
//
//class RequestsTableViewControllerWithWorker: UITableViewController {
//    
...
//            
//            let thisRealm: Realm? = try! Realm(configuration: YPB.realmConfig)
//            
//            if let requests = thisRealm?.objects(Request.self).filter("played = false") {
//                self?.worker = RequestsObserver(observing: requests) { [weak self] changes in
//                    switch changes {
