...
class RTVC_reformed: UITableViewController {
	...
	var requests: Results<Request>? {
		...
		guard YPB.realmSynced != nil else { return nil }
		
		let allRequests = YPB.realmSynced.objects(Request.self)
		...
		PH.requestsPredicates = NSCompoundPredicate(andPredicateWithSubpredicates: predicates)
	...
	func observeChanges() {
		...
		let blockToExecuteOnceRealmIsSet: (Notification)->Void = { [weak self] _ in
		...
		realmSetObserver = NotificationCenter.default.addObserver(forName: NSNotification.Name("realm set"), object: nil, queue: OperationQueue.main, using: blockToExecuteOnceRealmIsSet)
	}
